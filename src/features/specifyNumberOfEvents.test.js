...
defineFeature(feature, test => {
    test('If user hasn’t specified a number, the default number of displayed events will be 32', ({ given, and, when, then }) => {
        let AppDOM;
        ...
        then(/^the Number Of Events input field should display (\d+) by default$/, (arg0) => {
            const NumberOfEventsDOM = AppDOM.querySelector('#number-of-events');
            const numberOfEventsInput = within(NumberOfEventsDOM).queryByRole('textbox');
            expect(numberOfEventsInput.value).toBe("32");
        });

        and(/^the number of events in the list should be (\d+) by default$/, (arg0) => {
